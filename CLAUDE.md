...
- **DotNetWorkQueue** v0.12.0 + transport-specific packages (including `DotNetWorkQueue.Dashboard.Api`, `DotNetWorkQueue.Dashboard.Ui`)
...
- **SonarCloud** (`.github/workflows/sonarcloud.yml`) — Windows. Mirrors the core DotNetWorkQueue `sonarcloud.yml`: one `dotnet-sonarscanner` session wraps builds of all 8 solutions (SampleShared first). Project key `blehnen_DotNetWorkQueue.Samples`, org `blehnen`, needs the `SONAR_TOKEN` secret. Copy-paste detection is disabled for the five transport sample folders — the samples are deliberately parallel per transport (Sonar measured 48.9% duplication repo-wide unexcluded, vs. a 3% gate threshold on new code) and would otherwise fail the Duplications gate on every PR. `SampleShared`, `IntegrationTests`, and `DashBoard.Api` keep full CPD signal. Coverage is excluded because nothing in this repo measures it.
