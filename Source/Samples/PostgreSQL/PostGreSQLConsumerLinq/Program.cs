...
namespace PostGreSQLConsumerLinq
{
    class Program
    {
        static void Main(string[] args)
        {
                    ...
                    serviceRegister), options => Injectors.SetOptions(options, SharedConfiguration.EnableChaos)))
            {
                    ...
                    createQueueContainer.GetQueueCreation<PostgreSqlMessageQueueCreation>(queueConnection))
                {
                    if (!createQueue.QueueExists)
                    {
                        ...
                        Log.Error(
                            "Could not find {ConnectionString}. Verify that you have run the producer, which will create the queue", connectionString);
            ...
            var dashboardClient = Injectors.StartDashboardRegistration(queueName, "PostGreSQLConsumerLinq");
#endif
                ...
                using (var scheduler = schedulerContainer.CreateTaskScheduler())
                {
                            ...
                            "PostgreSqlConsumerLinq", serviceRegister), options => Injectors.SetOptions(options, SharedConfiguration.EnableChaos)))
                    {
                            ...
                            queueContainer.CreateConsumerMethodQueueScheduler(queueConnection, factory))
                        {
                            ...
                            queue.Configuration.HeartBeat.UpdateTime = TimeSpan.FromSeconds(10); //set a heartbeat every 10 seconds
